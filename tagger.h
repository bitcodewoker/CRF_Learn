...
class TaggerImpl : public Tagger {
 public:
  ...
  void set_penalty(size_t i, size_t j, double penalty); // 额外惩罚项
 ...
 private:
  ...
  size_t          ysize_;       // 标签集的大小
  double          cost_;        // 当前cost，优化目标即为降低cost
  double          Z_;           // normalized常数
  size_t          feature_id_;  // 该tagger控制的featureid
  unsigned short  thread_id_;   // 该tagger所属的线程id
  FeatureIndex   *feature_index_;   // featureindex类的引用
  Allocator      *allocator_;       // allocator类的引用
  std::vector<std::vector <const char *> > x_;
  std::vector<std::vector <Node *> > node_;     // 节点
  std::vector<std::vector<double> > penalty_;   // 额外的惩罚项penalty，默认为0
  std::vector<unsigned short int>  answer_;     // 每个词标准答案的索引
  std::vector<unsigned short int>  result_;     // 预测结果
  whatlog       what_;
  string_buffer os_;
