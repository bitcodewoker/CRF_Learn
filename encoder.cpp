...
class CRFEncoderThread: public thread {
 public:
  TaggerImpl **x;               // Tagger数组，数量等于token数
  unsigned short start_i;       // 当前线程tagger起始坐标
  unsigned short thread_num;    // 线程数
  int zeroone;                  // 出现错误的tagger个数，用于计算句子出错率
  int err;                      // 出错总数，用于计算整体出错率
  size_t size;                  // tagger数
...
bool runCRF(const std::vector<TaggerImpl* > &x,     // token列表
            EncoderFeatureIndex *feature_index,     // 特征编号表
            double *alpha,                          // 特征函数权重w
            size_t maxitr,                          // 最大迭代次数
            float C,                                // cost factor
            double eta,                             // 阈值
            unsigned short shrinking_size,          // 未使用特征数
            unsigned short thread_num,              // 线程数
            bool orthant) {                         // 是否使用L1范数
  double old_obj = 1e+37;
  int    converge = 0;
  LBFGS lbfgs;
  std::vector<CRFEncoderThread> thread(thread_num); // 线程小队

  for (size_t i = 0; i < thread_num; i++) {         // 线程小队初始化
    thread[i].start_i = i;
  ...
  for (size_t itr = 0; itr < maxitr; ++itr) {       // 开始迭代
    for (size_t i = 0; i < thread_num; ++i) {       // 多线程训练
      thread[i].start();
    }

    for (size_t i = 0; i < thread_num; ++i) {       // 等待全部线程结束
      thread[i].join();
    }
    ...
    for (size_t i = 1; i < thread_num; ++i) {
      for (size_t k = 0; k < feature_index->size(); ++k) {
        thread[0].expected[k] += thread[i].expected[k];     // 对每个特征融合所有线程的梯度
      }
    }
    ...
    if (itr > maxitr || converge == 3) {    // diff三次小于阈值则认为收敛
      break;  // 3 is ad-hoc
    }

    if (lbfgs.optimize(feature_index->size(),   // 据说这个模块是用一个叫f2c的工具从FORTRAN代码转成的C代码，可读性并不好，也就不再深入了。
                       &alpha[0],
                       thread[0].obj,
...
bool Encoder::learn(const char *templfile,  // 模板文件
                    const char *trainfile,  // 訓練文件
                    const char *modelfile,  // 模型文件
                    bool textmodelfile,     // 是否輸出文本形式的模型
                    size_t maxitr,          // 最大迭代次數
                    size_t freq,            // 特征最低频次
                    double eta,             // 收敛阈值
                    double C,               // 权值共享系数
                    unsigned short thread_num,  // 线程数
  ...
  EncoderFeatureIndex feature_index;        // 特征索引
  Allocator allocator(thread_num);          // 线程管理与数据分配
  std::vector<TaggerImpl* > x;              // Tagger标注器群
...
#define WHAT_ERROR(msg) do {                                    \
  ...
  CHECK_FALSE(feature_index.open(templfile, trainfile))
      << feature_index.what();      // 读取数据

  {
    progress_timer pg;      // 计时器
    ...
    while (ifs) {
      ...
      if (!_x->read(&ifs) || !_x->shrink()) {   // 读取并建立特征
        WHAT_ERROR(_x->what());
      }
      ...
      _x->set_thread_id(line % thread_num);     // 分配线程id
  ...
  feature_index.shrink(freq, &allocator);       // 根据阈值过滤特征

  std::vector <double> alpha(feature_index.size());           // parameter
  std::fill(alpha.begin(), alpha.end(), 0.0);
  feature_index.set_alpha(&alpha[0]);                         // 初始化alpha
  ...
  if (!feature_index.save(modelfile, textmodelfile)) {  // 保存模型
