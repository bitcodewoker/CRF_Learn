...
void TaggerImpl::forwardbackward() {
  ...
  for (int i = 0; i < static_cast<int>(x_.size()); ++i) {   // 前向推alpha
    for (size_t j = 0; j < ysize_; ++j) {
      node_[i][j]->calcAlpha();
    }
  }

  for (int i = static_cast<int>(x_.size() - 1); i >= 0;  --i) {     // 后向推beta
    for (size_t j = 0; j < ysize_; ++j) {
      node_[i][j]->calcBeta();
    }
  }
  ...
  Z_ = 0.0;
  for (size_t j = 0; j < ysize_; ++j) {         // 计算normalized常数Z
...
double TaggerImpl::gradient(double *expected) {     // 根据前后向算法求最终梯度
  ...
  for (size_t i = 0;   i < x_.size(); ++i) {
    for (const int *f = node_[i][answer_[i]]->fvector; *f != -1; ++f) {
      --expected[*f + answer_[i]];      // 减去节点标准答案的计数。期望公式中应为模型期望减去观测期望，但由于训练集的绝对正确性，
                                        // 导致观测期望为1，所以直接--
    }
    s += node_[i][answer_[i]]->cost;  // UNIGRAM cost，累加w*phi(y, x)
    const std::vector<Path *> &lpath = node_[i][answer_[i]]->lpath;
    for (const_Path_iterator it = lpath.begin(); it != lpath.end(); ++it) {
      if ((*it)->lnode->y == answer_[(*it)->lnode->x]) {
        for (const int *f = (*it)->fvector; *f != -1; ++f) {
          --expected[*f +(*it)->lnode->y * ysize_ +(*it)->rnode->y];    // 减去边标准答案的计数。
        }
  ...
  viterbi();  // call for eval()，跑viterbi是为了评估模型准确度，决定是否终止
