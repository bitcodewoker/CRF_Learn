...
bool FeatureIndex::applyRule(string_buffer *os,     // 模板特征匹配
                             const char *p,
                             size_t pos,
                             const TaggerImpl& tagger) const {
  os->assign("");  // clear
...
bool FeatureIndex::buildFeatures(TaggerImpl *tagger) const {    // 根据模板提取特征并放到cache中
  string_buffer os;
  ...
  for (size_t cur = 0; cur < tagger->size(); ++cur) {   // 遍历获得全部unigram特征
    for (std::vector<std::string>::const_iterator it    // 遍历unigram模板的每个特征
  ...
  for (size_t cur = 1; cur < tagger->size(); ++cur) {   // 遍历获得全部bigram特征
